...
<style>
    ...
    .stApp {
        background: linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), 
                    url("https://i.pinimg.com/736x/15/e2/d3/15e2d3b030a670c0b1f971381fc95f4e.jpg");
...
def main():
    ...
    st.markdown("""
    <div style='background-color: rgba(52, 73, 94, 0.5); padding: 15px; border-radius: 5px;'>
            ...
            with filter_tab1:
                ...
                with col1:
                    ...
                    if not filter_distance.empty:
                        ...
                        st.dataframe(filter_distance[display_cols].sort_values('Jarak').reset_index(drop=True), use_container_width=True)
                ...
                with col2:
                    ...
                    if not filter_budget.empty:
                        ...
                        st.markdown('<div style="width:100%; overflow-x:auto;">', unsafe_allow_html=True)
                        st.dataframe(filter_budget[display_cols].sort_values(htm_col).reset_index(drop=True), use_container_width=True)
                        st.markdown('</div>', unsafe_allow_html=True)
                    else:
            ...
            with filter_tab2:
                ...
                if preferred_categories and len(preferred_categories) > 0:
                    ...
                    if not filter_category.empty:
                        display_cols = ['Objek Wisata', 'Kategori Wisata', 'Rating', 'Jarak', 'HTM Weekday', 'HTM Weekend']
                        st.markdown('<div style="width:100%; overflow-x:auto;">', unsafe_allow_html=True)
                        st.dataframe(filter_category[display_cols].sort_values('Rating', ascending=False).reset_index(drop=True), use_container_width=True)
                        st.markdown('</div>', unsafe_allow_html=True)
                else:
